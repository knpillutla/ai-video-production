...
17. **Mandatory Consultative Alignment & Feedback First (No Premature / Blind Code Edits):** When the user shares an idea, request, architectural preference, or direction, the agent must **first provide thoughtful feedback, engineering analysis, trade-off evaluation, and professional advice BEFORE making any code, file, or configuration changes**. Discuss the approach, pros, cons, and alternatives with the user. Finalize the exact plan based on the user's intent combined with the agent's advice, and proceed with code modifications only once aligned. Never blindly apply unilateral code changes without consultation.
