"""Fal.ai Tencent Hunyuan Video 1080p Video Motion Synthesis Adapter.

Features:
- Native 1080p resolution output (1920x1080) for pristine 4K Lanczos upscaling
- Open-source video architecture with high prompt adherence and physical dynamics
- Rate: $0.075 / second
...
_HUNYUAN_1080P_ENDPOINT = "https://queue.fal.run/fal-ai/hunyuan-video-image-to-video"
_POLL_INTERVAL_S = 4.0
_POLL_MAX_ATTEMPTS = 225  # Up to 15m timeout
...
class FalHunyuan1080pAdapter:
    """Tencent Hunyuan Video 1080p Image-to-Video generation adapter on Fal.ai."""

    def __init__(self, api_key: str | None = None, endpoint: str | None = None) -> None:
        self.api_key = api_key or _fal_api_key()
        self.endpoint = endpoint or _HUNYUAN_1080P_ENDPOINT
    ...
    async def generate_video(
        ...
        """Synthesize 1080p video motion using Tencent Hunyuan Video."""
        out = Path(output_path)
        out.parent.mkdir(parents=True, exist_ok=True)

        if out.exists() and out.stat().st_size > 500_000:
            logger.info(f"fal_hunyuan_1080p_cache_hit: {out.name} ({out.stat().st_size} B)")
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=15.0)) as client:
            if not status_url or not response_url:
                ...
                if sub.status_code not in (200, 201, 202):
                    raise RuntimeError(f"Hunyuan submit failed: {sub.status_code} - {sub.text[:200]}")
                sub_data = sub.json()
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=15.0)) as client:
            for i in range(_POLL_MAX_ATTEMPTS):
                ...
                if status == "COMPLETED":
                    ...
                    logger.info(f"fal_hunyuan_1080p_ok: {out.name} ({len(v_bytes)} B)")
                    return vid_url, out
    ...
    async def _fallback_local(self, out: Path, duration: int) -> tuple[str, Path]:
        ...
        cmd = [
            exe, "-y",
            "-f", "lavfi", "-i", f"color=c=0x0f172a:s=1920x1080:d={duration}:r=24",
...
fal_hunyuan_1080p_adapter = FalHunyuan1080pAdapter()

__all__ = ["FalHunyuan1080pAdapter", "fal_hunyuan_1080p_adapter"]
